...
fn human_description(error: &QasmSimError) -> Option<HumanDescription> {
    match error {
        ...
        QasmSimError::RedefinitionError {
            ...
            linesrc: (*source).into(),
            help: Some(format!(
                "first declaration happens in line {}",
                *previous_lineno
            )),
        }),
        QasmSimError::IndexOutOfBounds {
            ...
            linesrc: (*source).into(),
            help: Some(format!(
                "indices of register `{}` range from 0 to {} but the index is {}",
                symbol_name,
                size - 1,
                index
            )),
        ...
        QasmSimError::TypeMismatch {
            ...
            msg: format!(
                "mismatched types for symbol `{}`: expected \"{}\"",
        ...
        QasmSimError::RegisterSizeMismatch {
            ...
            let msg = if symbol_name == "measure" {
                ...
                format!(
                    "cannot apply gate `{}` to registers of differen size",
                    symbol_name
                )
            };
            Some(HumanDescription {
                ...
                endpos: None,
                help: Some(format!(
                    "expected registers of same size, found registers of sizes {}",
                    sizes_str.join(", ")
                )),
            })
        }
        QasmSimError::WrongNumberOfParameters {
            ...
            let qualifier = if *are_registers {
                "quantum registers"
            } else {
                "real parameters"
            };
            Some(HumanDescription {
                msg: format!(
                    "wrong number of {} passed to gate `{}`",
                    qualifier, symbol_name
                ),
                linesrc: (*source).into(),
                lineno: *lineno,
                startpos: 0,
...
fn list_of_choices(choices: &[String]) -> Option<String> {
    ...
    match len {
        ...
        _ => Some({
            ...
            let except_last: Vec<String> = choices
                .iter()
                .take(len - 1)
                .map(|item| (*item).clone())
                .collect();
