...
pub fn print(path: &mut PathBuf, result: &Run, options: &Options) {
  ...
  let prefix =
    path.file_name().expect("a valid file name")
    .to_str().expect("a valid name for the filename")
    .to_owned();
  ...
  if (options.statevector || options.probabilities) && options.shots.is_none() {
    ...
    let mut writer = csv::Writer::from_path(&path).expect("can open the file");
    let writer_ref = &mut writer;
    print_state(writer_ref, &result.statevector, &result.probabilities, &options).expect("writes");
  }
...
fn print_memory<W>(writer: &mut csv::Writer<W>, memory: &HashMap<String, u64>, options: &Options)
...
fn print_histogram<W>(writer: &mut csv::Writer<W>, histogram: &Histogram, options: &Options)
-> io::Result<()> where W: Write {
  print_memory_summary(writer, histogram, options, false)
}

fn print_memory_summary<W>(writer: &mut csv::Writer<W>, histogram: &Histogram, options: &Options, omit_count: bool)
-> io::Result<()> where W: Write {
  ...
  if integer { titles.push("Int value"); }
  if hexadecimal { titles.push("Hex value"); }
  if binary { titles.push("Bin value"); }
  if !omit_count { titles.push("Count"); }
  writer.write_record(&titles)?;

  for (key, hist) in histogram {
    for (value, count) in hist {
      let mut record: Vec<String> = vec![key.clone()];
      if integer { record.push(format!("{}", value)); }
      if hexadecimal { record.push(format!("0x{:x}", value)); }
      if binary { record.push(format!("0b{:b}", value)); }
      if !omit_count { record.push(format!("{}", count)); }
...
fn print_times<W>(writer: &mut csv::Writer<W>, times: &RunTimes)
-> io::Result<()> where W: Write {
